...
uses
  ...
  Forms,
  jclfileutils,
  JCLStrings,  VCL.Styles, VCL.Themes,
  JvProgressDialog,
  Menus,
  SysUtils,
  Windows;
...
type
  ...
  TGetVCLStyleList = function: PChar; stdcall;
  TApplyVCLStyle = function(StyleName: PChar): Boolean; stdcall;
...
type
  ...
  type
  TSetDLLStyle = function(const StyleName: PChar): Boolean; stdcall;

const
  minimum_major_version = 0;

const
  minimum_minor_version = 29;
...
var
  ...
  ListStyles: TGetVCLStylelist;
  globalfilterindex: integer;
  ApplyStyle: TApplyVCLStyle;
  SetApp: TSetAppProc;
...
procedure LoadAndApplyStyleFromDLL;
...
var
  DLLHandle: THandle;
  GetVCLStyleList: TGetVCLStyleList;
  StyleNames: PChar;
begin
  DLLHandle := LoadLibrary(pchar(basepath+'\Plugins\ThemeLibraryModule.dll'));
  if DLLHandle = 0 then
  begin
    ShowMessage('Failed to load ThemeLibraryModule.dll');
    Exit;
  end;

  try
    @GetVCLStyleList := GetProcAddress(DLLHandle, 'GetVCLStyleList');
    if Assigned(GetVCLStyleList) then
    begin
      StyleNames := GetVCLStyleList;
      Debugmsg('Available Styles:' + sLineBreak + StyleNames);
    end
    else
      debugmsg('Function GetVCLStyleList not found.');
  finally
...
begin
  ...
  if DLLHandle = 0 then
  begin
    ShowMessage('Failed to load '+basepath+'\plugins\ThemeLibraryModule.dll');
    Exit;
  end;

  try
    @GetVCLStyleList := GetProcAddress(DLLHandle, 'GetVCLStyleList');
    @ApplyVCLStyle := GetProcAddress(DLLHandle, 'ApplyVCLStyle');

    if Assigned(GetVCLStyleList) and Assigned(ApplyVCLStyle) then
    begin
      // Get the available styles
      StyleNames := GetVCLStyleList;
      ShowMessage('Available Styles:' + sLineBreak + StyleNames);

      // Ask user to enter a style name
      SelectedStyle := InputBox('Select Style', 'Enter style name:', '');

      // Apply the selected style
      if ApplyVCLStyle(PChar(SelectedStyle)) then
        ShowMessage('Style applied successfully!')
      else
        ShowMessage('Failed to apply style.');
    end
    else
      ShowMessage('DLL functions not found.');
  finally
    FreeLibrary(DLLHandle);
  end;
end;
