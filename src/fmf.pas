...
begin;
  ...
  if tempcondcheck = 'CHECK_CUSTOM_CODE' then
    temp_cond.check_type := CHECK_TYPE_CUSTOM_CODE
  else if tempcondcheck = 'CHECK_SKILL' then
    temp_cond.check_type := CHECK_TYPE_SKILL
  else if tempcondcheck = 'CHECK_STAT' then
    temp_cond.check_type := CHECK_TYPE_STAT
  else if tempcondcheck = 'CHECK_MONEY' then
    temp_cond.check_type := CHECK_TYPE_CHECKMONEY
  else if tempcondcheck = 'GLOBAL_VARIABLE' then
  begin;
    temp_cond.check_type := CHECK_TYPE_GLOBAL_VARIABLE;
    temp_cond.var_ptr := tempcondfield;
  end
  else if tempcondcheck = 'LOCAL_VARIABLE' then
  begin;
    temp_cond.check_type := CHECK_TYPE_LOCAL_VARIABLE;
    temp_cond.var_ptr := tempcondfield;

  end
  else if tempcondcheck = 'SCRIPT_VARIABLE' then
  begin;
    temp_cond.check_type := CHECK_TYPE_SCRIPT_VARIABLE;
    temp_cond.var_ptr := tempcondfield;
  end
  else if tempcondcheck = 'PLAYER_HAS_ITEM' then
  begin;
  ...
  temp_cond.resolved_code :=  tempcondssl else
  ...
  try
    new(currentdlg.nodes[currentdlg.nodecount].options
      ...
      [currentdlg.nodecount].optioncnt].conditioncnt]);
    ...
    case state of
      1:
        if ((CurrInputCh >= '0') and (CurrInputCh <= '9') or
          ...
          or (CurrInputCh >= CHR(196)) and (CurrInputCh <= CHR(197)) or
      ...
      7:
        if ((CurrInputCh <= CHR(12)) or (CurrInputCh >= CHR(14)) and
          (CurrInputCh <= '!') or (CurrInputCh >= '#') and (CurrInputCh <= '[')
          or (CurrInputCh >= ']')) then
        begin
...
begin
  inherited;
  fHashList := TmwStringHashList.Create(ISmallCrcHash, HashSecondaryTwo,
    IHashCompare);
  fHashList.AddString('AND', ANDSym, ANDSym);
  fHashList.AddString('ASSOCIATE_NODE', ASSOCIATE_underscoreNODESym,
    ASSOCIATE_underscoreNODESym);
  ...
  fHashList.AddString('CUSTOM_PROC', CUSTOM_underscorePROCSym,
    CUSTOM_underscorePROCSym);
  fHashList.AddString('DEFAULT_CONDITION', DEFAULT_underscoreCONDITIONSym,
    DEFAULT_underscoreCONDITIONSym);
  fHashList.AddString('DEFAULT_EVENT', DEFAULT_underscoreEVENTSym,
    DEFAULT_underscoreEVENTSym);
  fHashList.AddString('DEFINE_SKILL_CHECK',
    DEFINE_underscoreSKILL_underscoreCHECKS,
    DEFINE_underscoreSKILL_underscoreCHECKS);
  fHashList.AddString('DESCRIPTION', DESCRIPTIONSym, DESCRIPTIONSym);
  fHashList.AddString('DETAILED_DESC', DETAILED_underscoreDESCSym,
    DETAILED_underscoreDESCSym);
  fHashList.AddString('DIFFICULTY_MODIFIER', DIFFICULTY_underscoreMODIFIERSym,
    DIFFICULTY_underscoreMODIFIERSym);
  fHashList.AddString('ENUM', ENUMSym, ENUMSym);
  fHashList.AddString('EVAL', EVALSym, EVALSym);
  fHashList.AddString('EXEC_SCRIPT_CODE',
  ...
  fHashList.AddString('FIXED_PARAM_NAME',
  ...
  fHashList.AddString('INI_INDEX', INI_underscoreINDEXSym,
    INI_underscoreINDEXSym);
  fHashList.AddString('INSERT_CUSTOM_CODE',
  ...
  fHashList.AddString('ISRANDOMINTERVAL', ISRANDOMINTERVALSym,
    ISRANDOMINTERVALSym);
  fHashList.AddString('IS_WTG', IS_underscoreWTGSym, IS_underscoreWTGSym);
  fHashList.AddString('KNOWN_DESC', KNOWN_underscoreDESCSym,
    KNOWN_underscoreDESCSym);
  ...
  fHashList.AddString('LINK_NEXT', LINK_underscoreNEXTSym,
    LINK_underscoreNEXTSym);
  ...
  fHashList.AddString('REACTION_BAD', REACTION_underscoreBADSym,
    REACTION_underscoreBADSym);
  fHashList.AddString('REACTION_GOOD', REACTION_underscoreGOODSym,
    REACTION_underscoreGOODSym);
  fHashList.AddString('REACTION_NEUTRAL', REACTION_underscoreNEUTRALSym,
    REACTION_underscoreNEUTRALSym);
  fHashList.AddString('SKILL_CHECK', SKILL_underscoreCHECKSym,
    SKILL_underscoreCHECKSym);
  fHashList.AddString('SKILL_NUM', SKILL_underscoreNUMSym,
    SKILL_underscoreNUMSym);
  fHashList.AddString('SSL', SSLSym, SSLSym);
  fHashList.AddString('START_CONDITIONS', START_underscoreCONDITIONSSym,
    START_underscoreCONDITIONSSym);
  fHashList.AddString('TARGET_NODE', TARGET_underscoreNODESym,
    TARGET_underscoreNODESym);
  fHashList.AddString('TIMEEVENT', TIMEEVENTSym, TIMEEVENTSym);
  fHashList.AddString('TRUE', TRUESym, TRUESym);
  fHashList.AddString('UNKNOWN_DESC', UNKNOWN_underscoreDESCSym,
    UNKNOWN_underscoreDESCSym);
  fHashList.AddString('VALUE_TO_CHECK', VALUE_underscoreTO_underscoreCHECKSym,
    VALUE_underscoreTO_underscoreCHECKSym);
  fHashList.AddString('VAR', VARSym, VARSym);
  fHashList.AddString('VARS', VARSSym, VARSSym);
  fHashList.AddString('VAR_PARAM', VAR_underscorePARAMSym,
    VAR_underscorePARAMSym);
  CurrentCh := CapChAt;
  fStartState[0] := 28;
  fStartState[1] := 29;
  fStartState[2] := 29;
...
function TFMF.ErrorStr(const ErrorCode: integer; const Data: ansistring)
  : ansistring;
begin
  case ErrorCode of
    0:
...
begin
  ...
  Expect(_lbraceSym);
  Expect(_stringSym);
  new(currentdlg.timedevents[currentdlg.timedeventcnt].actionlines
    [currentdlg.timedevents[currentdlg.timedeventcnt].actioncnt]);
  currentdlg.timedevents[currentdlg.timedeventcnt].actionlines
    [currentdlg.timedevents[currentdlg.timedeventcnt].actioncnt].linedata :=
    parse_string(lexstring);
...
begin
  ...
  begin
    Get;
    Expect(_stringSym);
    currentdlg.nodes[currentdlg.nodecount].npctext_female :=
      parse_string(lexstring);;
  end;
  Expect(OPTIONSSym);
  Expect(_lbraceSym);
  ...
  begin
    _Options;
    while (fCurrentInputSymbol = INTSym) or (fCurrentInputSymbol = LINKFLAGSSym)
      or (fCurrentInputSymbol = REACTIONSym) or
      (fCurrentInputSymbol = GENDERSym) or
...
begin
  ...
  new(currentdlg.nodes[currentdlg.nodecount].skillchecks
    [currentdlg.nodes[currentdlg.nodecount].skillcheckcnt]);
  currentdlg.nodes[currentdlg.nodecount].skillchecks
    [currentdlg.nodes[currentdlg.nodecount].skillcheckcnt].check_proc_name :=
    lexstring;;
  Expect(_lbraceSym);
  ...
  currentdlg.nodes[currentdlg.nodecount].skillchecks
    [currentdlg.nodes[currentdlg.nodecount].skillcheckcnt].check_what :=
    StrToInt(lexstring);
  ...
  currentdlg.nodes[currentdlg.nodecount].skillchecks
    [currentdlg.nodes[currentdlg.nodecount].skillcheckcnt].modifier :=
    StrToInt(lexstring);;
  Expect(_semicolonSym);
  Expect(ONSUCCESSSym);
  ...
  currentdlg.nodes[currentdlg.nodecount].skillchecks
    [currentdlg.nodes[currentdlg.nodecount].skillcheckcnt].successnode :=
    lexstring;
  ...
  currentdlg.nodes[currentdlg.nodecount].skillchecks
    [currentdlg.nodes[currentdlg.nodecount].skillcheckcnt].failurenode :=
    lexstring;
...
begin
  ...
  begin
    ...
    else if (fCurrentInputSymbol = GLOBALSYM) then
    begin
      Get;
...
begin
  ...
  Expect(integer_Sym);
  currentdlg.customprocs[currentdlg.customproccnt].associatewithnode :=
    StrToInt(lexstring);
  Expect(_lbraceSym);
  Expect(_stringSym);
  currentdlg.customprocs[currentdlg.customproccnt].Lines :=
    currentdlg.customprocs[currentdlg.customproccnt].Lines +
    parse_string(lexstring) + #13#10;;
  while (fCurrentInputSymbol = _commaSym) do
  begin
    Get;
    Expect(_stringSym);
  ...
  Expect(_rbraceSym);
  if procexists(currentdlg.customprocs[currentdlg.customproccnt].Name) = False
  then
...
begin
  ...
  Expect(_stringSym);
  currentdlg.startconditions[currentdlg.startconditioncnt].goto_node :=
    parse_string(lexstring);;
  Expect(_lbraceSym);
  _Conds;
  while (fCurrentInputSymbol = _commaSym) do
...
begin
  ...
  currentdlg.startconditions[currentdlg.startconditioncnt].conditions
    [currentdlg.startconditions[currentdlg.startconditioncnt].condcnt].linkage
    := LINKAGE_NONE;;
  Expect(SSLSym);
  Expect(_stringSym);
  currentdlg.startconditions[currentdlg.startconditioncnt].conditions
    [currentdlg.startconditions[currentdlg.startconditioncnt].condcnt]
    .resultcode := parse_string(lexstring);;
  Expect(VAR_underscorePARAMSym);
  Expect(_stringSym);
  currentdlg.startconditions[currentdlg.startconditioncnt].conditions
    [currentdlg.startconditions[currentdlg.startconditioncnt].condcnt].parameter
    := parse_string(lexstring);;
  Expect(EVALSym);
  Expect(integer_Sym);
  currentdlg.startconditions[currentdlg.startconditioncnt].conditions
    [currentdlg.startconditions[currentdlg.startconditioncnt].condcnt].evaluator
    := StrToInt(lexstring);;
  Expect(VALUE_underscoreTO_underscoreCHECKSym);
  Expect(_stringSym);
  currentdlg.startconditions[currentdlg.startconditioncnt].conditions
    [currentdlg.startconditions[currentdlg.startconditioncnt].condcnt].compareto
    := parse_string(lexstring);;
  Expect(INI_underscoreINDEXSym);
  Expect(integer_Sym);
  currentdlg.startconditions[currentdlg.startconditioncnt].conditions
    [currentdlg.startconditions[currentdlg.startconditioncnt].condcnt]
    .index_of_selection := StrToInt(lexstring);
...
begin
  ...
  begin
    ...
    currentdlg.floatnodes[currentdlg.floatnodecount].Messages
      [currentdlg.floatnodes[currentdlg.floatnodecount].messagecnt] :=
      parse_string(lexstring);
...
begin
  ...
  currentdlg.nodes[currentdlg.nodecount].options
    [currentdlg.nodes[currentdlg.nodecount].optioncnt].link_to_proc := False;
  currentdlg.nodes[currentdlg.nodecount].options
    [currentdlg.nodes[currentdlg.nodecount].optioncnt]
    .link_to_skillcheck := False;

  currentdlg.nodes[currentdlg.nodecount].options
    [currentdlg.nodes[currentdlg.nodecount].optioncnt].genderflags := 0;
  currentdlg.nodes[currentdlg.nodecount].options
    [currentdlg.nodes[currentdlg.nodecount].optioncnt].conditioncnt := 0;
  currentdlg.nodes[currentdlg.nodecount].options
    [currentdlg.nodes[currentdlg.nodecount].optioncnt].intcheck := 4;;
  ...
  begin
    ...
    Expect(integer_Sym);
    currentdlg.nodes[currentdlg.nodecount].options
      [currentdlg.nodes[currentdlg.nodecount].optioncnt].intcheck :=
      StrToInt(lexstring);;
  end;
  ...
  begin
    ...
    begin
      ...
      currentdlg.nodes[currentdlg.nodecount].options
        [currentdlg.nodes[currentdlg.nodecount].optioncnt]
        .link_to_skillcheck := True;
      currentdlg.nodes[currentdlg.nodecount].options
        [currentdlg.nodes[currentdlg.nodecount].optioncnt].link_to_proc
        := False;;
    end
  ...
  begin
    ...
    begin
      Get;
      currentdlg.nodes[currentdlg.nodecount].options
        [currentdlg.nodes[currentdlg.nodecount].optioncnt].reaction :=
        REACTION_BAD;
    end
  ...
  begin
    ...
    begin
      Get;
      currentdlg.nodes[currentdlg.nodecount].options
        [currentdlg.nodes[currentdlg.nodecount].optioncnt].genderflags :=
        GENDER_FEMALE;
    end
  ...
  Expect(_stringSym);
  currentdlg.nodes[currentdlg.nodecount].options
    [currentdlg.nodes[currentdlg.nodecount].optioncnt].optiontext :=
    parse_string(lexstring);;
  Expect(LINKTOSym);
  Expect(_stringSym);
  currentdlg.nodes[currentdlg.nodecount].options
    [currentdlg.nodes[currentdlg.nodecount].optioncnt].nodelink :=
    parse_string(lexstring);;
  ...
  Expect(_stringSym);
  currentdlg.nodes[currentdlg.nodecount].options
    [currentdlg.nodes[currentdlg.nodecount].optioncnt].notes :=
    parse_string(lexstring);
...
(*
  ...
  I have to manually fix the code to accept the custom condition code
  since I don't have the source for the fmf parser
*)
...
begin
 ...
 debugmsg('Check field: '+lexstring);
 ...
 get;
     tempcondssl := parse_string(lexstring);
 debugmsg('Check field = ssl: '+lexstring);
  end;

   if (fCurrentInputSymbol < 16) { prevent range error } and
      (fCurrentInputSymbol in [_equal_equalSym, _bang_equalSym, _lessSym,
      _greaterSym, _greater_equalSym, _less_equalSym]) then
    begin
      case fCurrentInputSymbol of
        _equal_equalSym:
          begin
...
begin
  ...
  Expect(_lbraceSym);
  if (fCurrentInputSymbol = IMPORTSym) or (fCurrentInputSymbol = LOCALSym) or
    (fCurrentInputSymbol = EXPORTSym) or (fCurrentInputSymbol = GLOBALSYM) or
    (fCurrentInputSymbol = VARSym) then
  begin
    __variable;
    while (fCurrentInputSymbol = IMPORTSym) or (fCurrentInputSymbol = LOCALSym)
      or (fCurrentInputSymbol = EXPORTSym) or (fCurrentInputSymbol = GLOBALSYM)
      or (fCurrentInputSymbol = VARSym) do
    begin
      __variable;
    end;
  end;
