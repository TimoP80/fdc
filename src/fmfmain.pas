...
uses SysUtils, Classes, gvarparse, fmf, cocobase, codesnippets, Rtti,
  System.TypInfo,
  VCL.ComCtrls, condcheck, Controls, debugwnd, Dialogs, dwsCompiler,
  dwsDebugger,
  dwsExprs, dwsSymbols, dwsStack, dwsComp, ExtCtrls, fmxutils, Forms, Graphics,
  helpscreen, SplashScreen,
  ImgList, inifiles, JclFileUtils, JclStrings, jvcomponent, jvfindreplace,
  jvrichedit, Menus, VCL.Styles, VCL.Themes, Messages, parseinclude, pluginapi,
  shareddlgdata, StdCtrls, syncompletionproposal, synedittypes, variants,
  Windows, Grids,
  dwsVCLGUIFunctions;
...
type
  tform1 = class(TForm)
    ...
    procedure mxwebupdate1aftershowinfo(Sender: TObject;
    ...
    procedure func_unitfunctionsgetplayeroptionstringdataeval
      (info: tprograminfo);
    procedure func_unitfunctionsgetplayeroptionintegerdataeval
      (info: tprograminfo);
    procedure func_unitfunctionssetplayeroptionstringdataeval
      (info: tprograminfo);
    procedure func_unitfunctionssetplayeroptionintegerdataeval
      (info: tprograminfo);
    ...
    procedure func_unitVariablesbasepathReadVar(info: tprograminfo;
      var value: Variant);
    ...
    procedure func_unitFunctionsFormattedIntEval(info: tprograminfo);
  private
  public
  end;
...
begin
  ...
  begin
    ...
    dlgtranscripts[dlgtranscriptcnt].Lines
      [dlgtranscripts[dlgtranscriptcnt].linecnt] := 'NPC: (' + currentdlg.nodes
      [nodehistory[t]].nodename + ') ' + currentdlg.nodes
      [nodehistory[t]].npctext;
    if (player.gender = gender_female) and
      (currentdlg.nodes[nodehistory[t]].npctext_female <> '') then
      dlgtranscripts[dlgtranscriptcnt].Lines
        [dlgtranscripts[dlgtranscriptcnt].linecnt] := 'NPC: (' +
        currentdlg.nodes[nodehistory[t]].nodename + ') ' + currentdlg.nodes
        [nodehistory[t]].npctext_female;
    dlgtranscripts[dlgtranscriptcnt].Lines
      [dlgtranscripts[dlgtranscriptcnt].linecnt] :=
      stringreplace(dlgtranscripts[dlgtranscriptcnt].Lines
      [dlgtranscripts[dlgtranscriptcnt].linecnt], '<PC>', player.Name,
      [rfreplaceall]);
    dlgtranscripts[dlgtranscriptcnt].Lines
      [dlgtranscripts[dlgtranscriptcnt].linecnt] :=
      stringreplace(dlgtranscripts[dlgtranscriptcnt].Lines
      [dlgtranscripts[dlgtranscriptcnt].linecnt], '${dude_name}', player.Name,
      [rfreplaceall]);
    Inc(dlgtranscripts[dlgtranscriptcnt].linecnt);
    if (nodehistorypreviousnode[t] <> -1) and (nodehistoryoption[t] <> -1) then
    begin
      dlgtranscripts[dlgtranscriptcnt].Lines
        [dlgtranscripts[dlgtranscriptcnt].linecnt] := 'Player: ' +
        currentdlg.nodes[nodehistorypreviousnode[t]].options
        [nodehistoryoption[t]].optiontext;
      dlgtranscripts[dlgtranscriptcnt].Lines
        [dlgtranscripts[dlgtranscriptcnt].linecnt] :=
        stringreplace(dlgtranscripts[dlgtranscriptcnt].Lines
        [dlgtranscripts[dlgtranscriptcnt].linecnt], '<PC>', player.Name,
  ...
  dlgtranscripts[dlgtranscriptcnt].histname :=
    format('%s_%0.3d_%s.txt',
    [extractfiledir(ParamStr(0)) + '\dialogue\history\' +
    extractfilename(changefileext(currentdlg.filename, '')), dlgtranscriptcnt,
    datestr]);
  debugmsg('Transcript #' + IntToStr(dlgtranscriptcnt) + ' generated.');
  Inc(dlgtranscriptcnt);
end;
...
begin
  ...
  debugmsg('disposed ' + IntToStr(currentdlg.nodecount) + ' nodes.');
end;
...
begin
  ...
  begin
    ...
    currentdlg.nodes[nodenum].options[i].optiontext := srcnode^.options[i]
      .optiontext;
    currentdlg.nodes[nodenum].options[i].nodelink :=
      srcnode^.options[i].nodelink;
...
begin
  ...
  begin
    new(Result^.nodes[t]);
    Result^.nodes[t]^ := currentdlg.nodes[t]^;
    Result^.nodes[t]^.optioncnt := currentdlg.nodes[t].optioncnt;
    // Copy options
    for j := 0 to currentdlg.nodes[t]^.optioncnt - 1 do
    begin
      new(Result^.nodes[t]^.options[j]);
      Result^.nodes[t]^.options[j]^ := currentdlg.nodes[t]^.options[j]^;
    end;

  end;

  Result^.timedeventcnt := currentdlg.timedeventcnt;
  for t := 0 to currentdlg.timedeventcnt - 1 do
  begin
    new(Result^.timedevents[t]);
    Result^.timedevents[t]^ := currentdlg.timedevents[t]^;
    // Copy actions
    Result^.timedevents[t]^.actioncnt := currentdlg.timedevents[t]^.actioncnt;
    for j := 0 to currentdlg.timedevents[t]^.actioncnt - 1 do
    begin
      new(Result^.timedevents[t]^.actionlines[j]);
      Result^.timedevents[t]^.actionlines[j]^.linedata := currentdlg.timedevents
        [t]^.actionlines[j]^.linedata;
    end;
  end;
  Result^.default_cond := currentdlg.default_cond;
  Result^.startconditioncnt := currentdlg.startconditioncnt;

  for t := 0 to currentdlg.startconditioncnt - 1 do
  begin
    new(Result^.startconditions[t]);
    Result^.startconditions[t]^ := currentdlg.startconditions[t]^;
    Result^.startconditions[t]^.goto_node := currentdlg.startconditions[t]
      ^.goto_node;

    Result^.startconditions[t]^.condcnt := currentdlg.startconditions
      [t]^.condcnt;

    for j := 0 to Result^.startconditions[t]^.condcnt - 1 do
    begin
      new(Result^.startconditions[t]^.conditions[j]);
      Result^.startconditions[t]^.conditions[j]^ := currentdlg.startconditions
        [t].conditions[j]^;
    end;

  end;
  Result^.customproccnt := currentdlg.customproccnt;
  for t := 0 to currentdlg.customproccnt - 1 do
  begin
    new(Result^.customprocs[t]);
    Result^.customprocs[t]^ := currentdlg.customprocs[t]^;
  end;

  Result^.floatnodecount := currentdlg.floatnodecount;
  for t := 0 to currentdlg.floatnodecount - 1 do
  begin
    new(Result^.floatnodes[t]);
    Result^.floatnodes[t]^ := currentdlg.floatnodes[t]^;
  end;
  Result^.varcnt := currentdlg.varcnt;
  for t := 0 to currentdlg.varcnt - 1 do
  begin
    new(Result^.variables[t]);
    Result^.variables[t]^ := currentdlg.variables[t]^;
  end;

end;

function GetStyle: shortstring; stdcall;
begin
  Result := cfg.readstring('Global Config', 'StyleName', '');
end;
...
begin
  ...
  begin
    ...
    form1.syncompletionproposal2.itemlist.add(gvars[x].Name + ' (' +
      IntToStr(x) + ')');
  end;

  for x := 0 to currentdlg.floatnodecount - 1 do
  begin
    form1.syncompletionproposal2.insertlist.add
      (currentdlg.floatnodes[x].nodename);
    form1.syncompletionproposal2.itemlist.add
      (currentdlg.floatnodes[x].nodename);
  end;
  ...
  begin
    ...
    begin
      form1.syncompletionproposal2.insertlist.add
        ('LVAR_' + currentdlg.variables[x].Name);
      form1.syncompletionproposal2.itemlist.add
        ('LVAR_' + currentdlg.variables[x].Name);
    end
    else
...
procedure SetGlobalFontSize(ASize: Integer; AName: string);
var
  I: Integer;
begin
  for I := 0 to Screen.FormCount - 1 do
  begin
    Screen.Forms[I].Font.Size := ASize;
    Screen.Forms[I].Font.Name := AName;
...
begin
  ...
  randomize;
  data_dir := extractfiledir(ParamStr(0));
  basepath := extractfiledir(ParamStr(0));
  booting_up := True;
  resetlog;
  new(player);
  UpdateSplashLabel('Loading config files...');
  lookuplist := TStringList.Create;
  condcfg := tinifile.Create(data_dir + '\conditions.ini');
  dlgtranscriptcnt := 0;
  cfg := tinifile.Create(data_dir + '\fdc.ini');
  globalfontsize := cfg.readinteger('Global Config', 'FontSize', 8);
  globalfontname := cfg.readstring('Global Config', 'FontName', 'Tahoma');

 SetGlobalFontSize(globalfontsize, globalfontname);
  ...
  numskillsimulations := cfg.readinteger('Dialogue Engine',
    'NumSkillSimulations', 20);
  timer1.interval := cfg.readinteger('Global Config',
    'AutoSaveInterVal', 300000);

  TStyleManager.TrySetStyle(cfg.readstring('Global Config', 'StyleName',
  ...
  debugmsg('Starting up Fallout Dialogue Creator...');
  ...
  evalskillchecks := cfg.readbool('Dialogue Engine',
    'EvaluateSkillChecks', True);
  ...
  player.luck := cfg.readinteger('Player Character', 'Luck', 5);
  player.dude_caps := cfg.readinteger('Player Character', 'Dude_Caps', 0);
  for t := 0 to 17 do
  begin
    player.skills[t].Name := cfg.readstring('Skills',
      'Skill' + IntToStr(t + 1), '');
  ...
  case runtimedlgedit of
    True:
      debugmsg('Runtime DLG Editing ON');
    False:
      debugmsg('Runtime DLG Editing OFF');
  end;
  defsmartint := cfg.readinteger('Global Config',
  ...
  case checkforupdatesatstartup of
    True:
      debugmsg('Autoupdate checking ON');
    False:
      debugmsg('Autoupdate checking OFF');
  end;
  form6 := tform6.Create(form1);

  UpdateSplashLabel('Scanning plugins: Net');
  scanforplugins(basepath + '\plugins\Net_*.dll', form1.mainmenu1, file1,
    form1.pluginexec, False, preferences1.menuindex + 1,
    'Loading internet-related plugins...');
  UpdateSplashLabel('Scanning plugins: Utility');
  scanforplugins(basepath + '\plugins\Utility_*.dll', form1.mainmenu1, plugins1,
    form1.pluginexec, True, -1, 'Loading generic plugins...');
  UpdateSplashLabel('Scanning plugins: Development tools');
  scanforplugins(basepath + '\plugins\DEV_*.dll', form1.mainmenu1, plugins1,
    form1.pluginexec, False, -1, 'Loading development tool plugins...');
  UpdateSplashLabel('Scanning plugins: Export plugins');
  scanforplugins(basepath + '\plugins\Export_*.dll', form1.mainmenu1,
    exporthtml1, form1.pluginexec, True, -1, 'Loading export plugins...');
  UpdateSplashLabel('Scanning plugins: Import plugins');
  scanforplugins(basepath + '\plugins\Import_*.dll', form1.mainmenu1, import1,
    form1.pluginexec, True, -1, 'Loading import plugins...');
  currentdlg.nodecount := 0;
...
begin
  ...
  begin
    ...
    form1.Caption := 'Fallout Dialogue Creator - [' +
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if IsValidNodeName(currentdlg.nodes[f].nodename, errormsg) = False
            then
            begin
              errorlist.add(currentdlg.nodes[f].nodename + ' is not valid - ' +
                errormsg);

              oldname := currentdlg.nodes[f].nodename;
              currentdlg.nodes[f].nodename :=
                stringreplace(currentdlg.nodes[f].nodename, '-', '_',
                [rfreplaceall]);
              currentdlg.nodes[f].nodename :=
                stringreplace(currentdlg.nodes[f].nodename, ' ', '_',
                [rfreplaceall]);
              currentdlg.nodes[f].nodename :=
                stringreplace(currentdlg.nodes[f].nodename, '''', '',
                [rfreplaceall]);
              if currentdlg.nodes[f].nodename <> '' then
              begin
                errorlist.add('Node renamed to ' + currentdlg.nodes[f]
                  .nodename);
            ...
            begin
              ...
              begin
                errorlist.add('[WARNING] Player option ' + IntToStr(x) +
                  ...
                  ' links to a non-existent node "' + currentdlg.nodes[f]
                  .options[x].nodelink +
          ...
          begin
            if IsValidNodeName(currentdlg.floatnodes[f].nodename, errormsg) = False
            then
            begin
              ...
              currentdlg.floatnodes[f].nodename :=
                stringreplace(currentdlg.floatnodes[f].nodename, ' ', '_',
                [rfreplaceall]);
              currentdlg.floatnodes[f].nodename :=
                stringreplace(currentdlg.floatnodes[f].nodename, '''', '',
                [rfreplaceall]);

              errorlist.add('Node renamed to ' + currentdlg.floatnodes[f]
                .nodename);
              Inc(numerrors);
        ...
        form1.statusbar1.simpletext := 'Dialogue ' + currentfile + ', ' +
          IntToStr(currentdlg.nodecount) + ' nodes';
      ...
      begin
        form7.memo1.Text :=
          'There were errors while parsing the dialogue script. ' + #13#10 +
          zerrorstr;
        form7.showmodal;
      end;
    end
    ...
    begin
      ...
      if plugind <> -1 then
      begin
        debugmsg('plugin: ' + IntToStr(plugind) + ': ' + dllplugins[plugind]
          .filename);
...
begin
  ...
  begin
        ...
        begin
          case fld of
            check_field_skill_smallguns:
              againstvalue := player.skills[0].value;
            check_field_skill_bigguns:
              againstvalue := player.skills[1].value;
            check_field_skill_energyweapons:
              againstvalue := player.skills[2].value;
            check_field_skill_unarmed:
              againstvalue := player.skills[3].value;
            check_field_skill_melee:
              againstvalue := player.skills[4].value;
            check_field_skill_throwing:
              againstvalue := player.skills[5].value;
            check_field_skill_firstaid:
              againstvalue := player.skills[6].value;
            check_field_skill_doctor:
              againstvalue := player.skills[7].value;
            check_field_skill_sneak:
              againstvalue := player.skills[8].value;
            check_field_skill_lockpick:
              againstvalue := player.skills[9].value;
            check_field_skill_steal:
              againstvalue := player.skills[10].value;
            check_field_skill_traps:
              againstvalue := player.skills[11].value;
            check_field_skill_science:
              againstvalue := player.skills[12].value;
            check_field_skill_repair:
              againstvalue := player.skills[13].value;
            check_field_skill_speech:
              againstvalue := player.skills[14].value;
            check_field_skill_barter:
              againstvalue := player.skills[15].value;
            check_field_skill_gambling:
              againstvalue := player.skills[16].value;
            check_field_skill_outdoorsman:
              againstvalue := player.skills[17].value;
          end;
        end;
      check_type_stat:
        begin
          case fld of
            check_field_stat_st:
              againstvalue := player.strength;
            check_field_stat_pe:
              againstvalue := player.perception;
            check_field_stat_en:
              againstvalue := player.endurance;
            check_field_stat_ch:
              againstvalue := player.charisma;
            check_field_stat_in:
              againstvalue := player.intelligence;
            check_field_stat_ag:
              againstvalue := player.agility;
            check_field_stat_lk:
              againstvalue := player.luck;
          end;
        end;
    end;
    ...
    if evalresult = True then
      ...
      Inc(fieldsfalse);
    debugmsg('CheckCondition: ' + theopt.conditions[t].resolved_code + ' == ' +
...
begin
  ...
  if (opt.intcheck > 0) and (player.intelligence >= opt.intcheck) then
    intcheckpassed := True;
  if (opt.intcheck < 0) and
    (player.intelligence <= (opt.intcheck + (opt.intcheck * -2))) then
  begin
    intcheckpassed := True;
  end
  else if (opt.intcheck < 0) and
    (player.intelligence > (opt.intcheck + (opt.intcheck * -2))) then
  begin
...
begin
  ...
  begin
    messagedlg
      ('Dialogue is assumed to end here since there are no player options available.',
  ...
  begin
    ...
    begin
      ...
      s.subitems[0] := stringreplace(s.subitems[0], '<PC>', player.Name,
        [rfreplaceall]);
      s.subitems[0] := stringreplace(s.subitems[0], '${dude_name}', player.Name,
        [rfreplaceall]);
      s.subitems.add(currentdlg.nodes[currentnode].options[t].nodelink);
      s.subitems.add(reacttostr(currentdlg.nodes[currentnode].options[t]
        .reaction));
    end
    else
      Inc(hiddenopts);
  ...
  if hiddenopts > 0 then
    form1.hiddenopts.Caption := format('There are %d hidden player options.',
      [hiddenopts])
  else
    form1.hiddenopts.Caption := '';
end;

procedure setnodehistory(ind: integer; thenode: integer;
  playeropt: integer = -1; previous: integer = -1);
begin
  if thenode <> -1 then
  begin
    ...
    nodehistory[ind] := thenode;
    nodehistoryoption[ind] := playeropt;
    nodehistorypreviousnode[ind] := previous;
    debugmsg('SetNodeHistory(''' + IntToStr(ind) + ''',''' + currentdlg.nodes
      [thenode].nodename + ''') Playeroption: ' + IntToStr(playeropt));
  end;
end;
...
begin
  ...
  begin
    ...
    nodehistorypreviousnode[nodehistorysize] := previous;
    if playeropt <> -1 then
      debugmsg('AddNodeToHistory(''' + currentdlg.nodes[thenode].nodename +
        ''') Playeroption: ' + IntToStr(playeropt) + ' TEXT=' + currentdlg.nodes
        [previous].options[playeropt].optiontext)
    else
      debugmsg('AddNodeToHistory(''' + currentdlg.nodes[thenode].nodename +
...
begin
  ...
  begin
    ...
    begin
      messagedlg('Not going to run dialogue from a node with' + #13 + #10 +
        'no name.' + #13 + #10 + '' + #13 + #10 +
    ...
    begin
      messagedlg(format('Error occured: Node ''%s'' not found.',
        [combobox1.Text]), mtWarning, [mbOK], 0);
      exit;
    end;
    nodehistorysize := 0;
...
begin
  ...
  if showdebug = True then
    debugmsg('Performing skill check ' + currentdlg.nodes[nodeind].skillchecks
      [ind].check_proc_name);
  ...
  skill_value := skill_value - (currentdlg.nodes[nodeind].skillchecks[ind]
    .modifier * -1);
  roll_value := 0;
  dice_roll := 1 + random(6);
  dice_skill := 1 + random(6);
  random_threat := random(10) * (currentdlg.nodes[nodeind].skillchecks[ind]
    .modifier * -1);
  ...
  if showdebug = True then
    debugmsg('Rolled (' + IntToStr((currentdlg.nodes[nodeind].skillchecks[ind]
      .modifier * -1)) + ' + ' + IntToStr(skill_rollval) + ' + (random_threat='
      + IntToStr(random_threat) + ') = ' + IntToStr(roll_value) + ') against ' +
      IntToStr(skill_value));
  ...
  begin
    ...
    Result := getnodeindex(currentdlg.nodes[nodeind].skillchecks[ind]
      .failurenode);
  end
  else if roll_value <= skill_value then
  begin
    if showdebug = True then
    begin
...
begin
  ...
  begin
    ...
    if x = getnodeindex(currentdlg.nodes[nodeind].skillchecks[ind].successnode)
    then
      Inc(successes)
    else if x = getnodeindex(currentdlg.nodes[nodeind].skillchecks[ind]
      .failurenode) then
      Inc(failures);
  end;
...
begin
  ...
  begin
    ...
    if (currentdlg.nodes[currentnode].options[optionind]
      .link_to_skillcheck = True) then
    begin
      if (evalskillchecks = True) then
        currentnode := checkskills(currentdlg.nodes[currentnode].options
          [optionind].nodelink)
      else
      begin
        skind := getskillcheckindex(currentdlg.nodes[currentnode].options
          [optionind].nodelink, _nodeind);
        ...
        begin
          ...
          form27.jvmarkuplabel1.Text :=
            format(Text, [currentdlg.nodes[_nodeind].skillchecks[skind]
            .check_proc_name, statname, currentdlg.nodes[_nodeind].skillchecks
            [skind].modifier, currentdlg.nodes[_nodeind].skillchecks[skind]
            .successnode, currentdlg.nodes[_nodeind].skillchecks[skind]
            .failurenode, (successes / numskillsimulations) * 100,
            numskillsimulations, (failures / numskillsimulations) * 100,
            numskillsimulations]);
        end
        else
        begin
          form27.jvmarkuplabel1.Text :=
            format(Text, [currentdlg.nodes[_nodeind].skillchecks[skind]
            .check_proc_name, player.skills[currentdlg.nodes[_nodeind]
            .skillchecks[skind].check_what - 1].Name,
            currentdlg.nodes[_nodeind].skillchecks[skind].modifier,
        ...
        begin
          case state of
            state_go_success:
              begin
                currentnode :=
                  getnodeindex(currentdlg.nodes[_nodeind].skillchecks[skind]
                  .successnode);
                debugmsg('Going to node: ' + currentdlg.nodes[_nodeind]
                  .skillchecks[skind].successnode);
              end;
            state_go_failure:
              begin
                currentnode :=
      ...
      currentnode := currentdlg.nodes[currentnode].options[optionind]
        .noderesolved;
    if nodehistorysize > 0 then
    begin
    ...
    begin
      ...
      begin
        messagedlg('Dialogue ends here.' + #13 + #10 + '' + #13 + #10 +
          '(Runtime dialogue editing = OFF)', mtInformation, [mbOK], 0);
      ...
      form8.nodelabel.Caption := currentdlg.nodes[prevnode].options
        [optionind].nodelink;
      if form8.nodelabel.Caption = '' then
        form8.nodelabel.Caption := '<empty>';
      form8.notesmemo.Text := currentdlg.nodes[prevnode].options
        [optionind].notes;
      ...
      if (currentdlg.nodes[prevnode].options[optionind].nodelink <> 'done') and
        (currentdlg.nodes[prevnode].options[optionind].nodelink <> 'combat')
      then
      begin
      ...
      begin
        form3.edit1.Text := currentdlg.nodes[prevnode].options
          [optionind].nodelink;
        ...
        begin
          ...
          currentdlg.nodes[prevnode].options[optionind].nodelink :=
            globalnodename;
        end;
        ...
        if currentnode = -1 then
        begin
          debugmsg('Error: NodeIndex is still -1, can''t continue.');
          messagedlg
            ('The node reference is still null, remaining in current node.',
            mtWarning, [mbOK], 0);
          currentnode := prevnode;
        end
        else
...
begin
  ...
  begin
    ...
    begin
      form13.jvrichedit1.addformattext(currentdlg.npcname + ': ', [fsbold],
        'Arial', $112211);
      form13.jvrichedit1.addformattext(copy(dlgtranscripts[ind].Lines[t],
        pos(':', dlgtranscripts[ind].Lines[t]) + 2,
        length(dlgtranscripts[ind].Lines[t])), [fsbold], 'Tahoma', $A20012);
    end
    else
    begin
      form13.jvrichedit1.addformattext(player.Name + ': ', [fsbold],
        'Arial', $000000);
      form13.jvrichedit1.addformattext(copy(dlgtranscripts[ind].Lines[t],
        pos(':', dlgtranscripts[ind].Lines[t]) + 2,
...
begin
  ...
  begin
    ...
    begin
      ...
      setnodehistory(nodehistorysize - 1, nodehistory[nodehistorysize - 1], -1,
        currentnode);
  end;
  button5.Enabled := False;
  button4.Enabled := False;
  ...
  begin
    ...
    begin
      ...
      if currentdlg.nodes[currentnode].options[optind].noderesolved <> -1 then
        memo2.Text := 'Leads to node ' + currentdlg.nodes[currentnode].options
          [optind].nodelink
      else if (currentdlg.nodes[currentnode].options[optind].noderesolved = -1)
        and (currentdlg.nodes[currentnode].options[optind]
        .link_to_skillcheck = True) then
      begin
        memo2.Text :=
          'Linked to a skill check [see Dialogue Editor->Skill Checks->' +
          currentdlg.nodes[currentnode].options[optind].nodelink + ']';
      end
      else if (currentdlg.nodes[currentnode].options[optind].noderesolved = -1)
        and (currentdlg.nodes[currentnode].options[optind].link_to_proc = True)
      then
      begin
        memo2.Text := 'Linked to a custom procedure ' + currentdlg.nodes
          [currentnode].options[optind].nodelink + '.';
      end
      else if (currentdlg.nodes[currentnode].options[optind].nodelink <> 'done')
        and (currentdlg.nodes[currentnode].options[optind].nodelink <> 'combat')
      then
        memo2.Text := 'Unresolved node ' + currentdlg.nodes[currentnode].options
          [optind].nodelink
      else if (currentdlg.nodes[currentnode].options[optind].nodelink = 'done')
      then
        memo2.Text := 'This option will end the dialogue.'
      else if (currentdlg.nodes[currentnode].options[optind].nodelink = 'combat')
      then
        memo2.Text := 'This option will initiate combat.';
      memo2.Text := memo2.Text + #13#10 + currentdlg.nodes[currentnode].options
        [optind].notes + getconds(currentdlg.nodes[currentnode].options
        [optind]^);
    end;
  end;
end;
...
begin
  executefile(getcurrentdir + '\doc\fmfdlgtool.chm', '', getcurrentdir + '\doc',
    sw_shownormal);
end;
...
{$IFDEF BETARELEASE}
  form5.label4.Caption :=
    'Note: This is a beta release. This means that not everything has been thoroughly tested. If you encounter a bug, please report it to me via e-mail at tpitkane@gmail.com';
...
procedure tform1.mxwebupdate1download(Sender: TObject;
  total, downloaded: integer);
begin
  ...
  form6.label1.Caption := format('Downloaded %d bytes of %d bytes',
    [downloaded, total]);
  if downloaded = total then
    form6.hide;
end;
...
procedure tform1.mxwebupdate1getclientfilename(Sender: TObject;
  var filename: string);
begin
...
procedure tform1.mxwebupdate1clientfileexists(Sender: TObject;
  var filename: string; var overwrite: boolean);
begin
  overwrite := (messagedlg(format('File ''%s'' already exists. Overwrite?',
    [filename]), mtWarning, [mbYes, mbNo], 0) = mrYes);
end;
...
begin
  ...
  form1.memo1.Text := stringreplace(form1.memo1.Text, '<PC>', player.Name,
    [rfreplaceall]);
...
begin
  ...
  else if currentfile = 'untitled' then
  begin
...
begin
  ...
  begin
    if outputwnd = nil then
      errorlist := '[ExecuteScriptConsole]' + #13#10 +
        'Script data has errors:';
...
begin
  ...
  begin
    messagedlg('Cannot close program while starting up!' + #13 + #10 + '' + #13
      + #10 + '"Patience, young padawan..."', mtError, [mbOK], 0);
    canclose := False;
    exit;
  end;
  if cfg.readbool('Global Config', 'Savenodehistory', True) = True then
  ...
  begin
    msgresult := messagedlg('Save changes to dialogue ''' + currentfile + '''' +
      #13 + #10 + 'before closing Fallout Dialogue Creator?', mtConfirmation,
      [mbYes, mbNo, mbCancel], 0);
    if msgresult = mrYes then
    begin
  ...
  else if deinitresult = True then
  begin
...
begin
  nodename := info.vars['nodename'].ValueAsString;
  npctext := info.vars['npctext'].ValueAsString;
  npctextfemale := info.vars['npctext_female'].ValueAsString;
  notes := info.vars['notes'].ValueAsString;
...
var
  ...
  nodeind: integer;
begin
  destnodename := info.vars['destnodename'].ValueAsString;
  opttext := info.vars['opttext'].ValueAsString;
  nodelink := info.vars['nodelink'].ValueAsString;
  notes := info.vars['notes'].ValueAsString;
...
begin
  ...
  zerrorstr := zerrorstr + format('Error on line %d column %d:' + #13 + #10 +
    '%s' + #13 + #10 + '%s', [error.line, error.col,
    fmf1.errorstr(error.errorcode, ''), errordata]);
  s.Free;
end;

procedure tform1.mxwebupdate1aftershowinfo(Sender: TObject;
  checkforupdate: boolean);
begin
...
var
  x, j, i: integer;
begin
  i := getnodeindex(info.vars['Oldname'].ValueAsString);
  if i <> -1 then
  begin
    currentdlg.nodes[i].nodename := info.vars['Newname'].ValueAsString;
    WNDOutput('Renamed ' + info.vars['Oldname'].ValueAsString + ' to ' +
      info.vars['Newname'].ValueAsString);
    RemapNodes(info.vars['Oldname'].ValueAsString,
      info.vars['Newname'].ValueAsString);
    requestpluginrefresh;
  end;
end;
...
var
  ...
  newdata: string;
begin
  nodeind := info.vars['Nodeindex'].value;
  optind := info.vars['OptionIndex'].value;
  newdata := info.vars['Newdata'].ValueAsString;
...
begin
  ...
  begin
    ...
    begin;
      debugmsg('FUNC: ' + func_unit.functions.items[f].Name +
        ' already exists.');
    end;
  end;
  for f := 0 to func_unit.functions.Count - 1 do
...
begin
  if dlgrunning = True then
    info.vars['result'].value := currentdlg.nodes[currentnode].nodename
  else
    info.vars['result'].value := 'ERROR';
  WNDOutput('CurrentNode: ' + info.vars['result'].value);
end;
...
begin
  if dlgrunning = True then
  begin
    currentnode := getnodeindex(info.vars['name'].ValueAsString);
    if currentnode <> -1 then
    begin
      NodeUpdate;
      WNDOutput('Current node set to ' + info.vars['name'].ValueAsString);
    end;
  end;
end;
...
var
  n: integer;
begin
  form10.runtimedlgedit.Checked := cfg.readbool('Global Config',
    'RuntimeDLGEditing', False);
  form10.last_trans.Checked := cfg.readbool('Global Config',
    'ShowLastDialogue', True);
  form10.jvdirectoryedit3.Text := cfg.readstring('Global Config',
    'HeadersPath', '');
  form10.checkbox2.Checked := evalskillchecks;
  form10.jvdirectoryedit4.Text := cfg.readstring('Global Config',
    'FO2Basepath', '');
  form10.globalfontsize.Text := IntToStr(globalfontsize);
  form10.autonodemap.Checked := autonodemap;
  form10.checkupdates.Checked := checkforupdatesatstartup;
  form10.JvFontComboBox1.itemindex:=form10.JvFontComboBox1.items.indexof(globalfontname);
  form10.compwarn.Checked := cfg.readbool('Global Config',
    'PluginCompatibilityWarnings', False);
  form10.jvdirectoryedit1.Text := cfg.readstring('Global Config',
    'DefaultSSLPath', '');
  form10.jvdirectoryedit2.Text := cfg.readstring('Global Config',
    'DefaultMSGPath', '');
  form10.evalcond.Checked := cfg.readbool('Dialogue Engine',
    'EvaluateConditions', True);
  form10.fo2datapath.Text := cfg.readstring('Global Config', 'FO2DataPath', '');
  form10.savenodehist.Checked := cfg.readbool('Global Config',
    'Savenodehistory', False);
  ...
  ListStyles;
  debugmsg('Active style = ' + TStyleManager.ActiveStyle.Name);
  form10.StyleList.ItemIndex := form10.StyleList.items.IndexOf
    (cfg.readstring('Global Config', 'StyleName',
    TStyleManager.ActiveStyle.Name));
  ...
  form10.jvspinedit1.value :=
    (cfg.readinteger('Global Config', 'AutoSaveInterval', 300000)
    div 60) div 1000;
  form10.checkbox3.Checked := form1.timer1.Enabled;
...
begin
  ...
  begin
    dlgeval := form10.evalcond.Checked;
    runtimedlgedit := form10.runtimedlgedit.Checked;
    debugmsg('Reloading global vars...');
    TStyleManager.TrySetStyle(cfg.readstring('Global Config', 'StyleName', ''));
    fo2datapath := cfg.readstring('Global Config', 'FO2DataPath', '');
    gvarparser := tgvarparse.Create(form1);
    globalfontsize := strtoint(form10.globalfontsize.text);
    globalfontname := form10.JvFontComboBox1.Text;
    SetGlobalFontSize(globalfontsize,globalfontname);
    cfg.writeinteger('Global Config','FontSize',globalfontsize);
    cfg.WriteString('Global Config','FontName',globalfontname);
   if fileexists(fo2datapath + '\data\VAULT13.GAM') then
    begin
      gvarparser.sourcefilename := fo2datapath + '\data\VAULT13.GAM';
      gvarparser.Execute;
...
begin
  info.vars['result'].value := cfg.readstring('Global Config',
    'DefaultSSLPath', '');
  WNDOutput('SSL Path = ' + info.vars['result'].value);
end;
...
begin
  info.vars['result'].value := cfg.readstring('Global Config',
    'DefaultMSGPath', '');
  WNDOutput('MSG Path = ' + info.vars['result'].value);
end;

procedure tform1.func_unitfunctionsdebugmsgeval(info: tprograminfo);
begin
  debugmsg(info.vars['message'].value);
end;
...
var
  s: integer;
begin
  WNDOutput('Launching editnode on ' + info.vars['node'].value);
  s := getnodeindex(info.vars['node'].value);
  if s = -1 then
  begin
    messagedlg('Node ' + info.vars['node'].value + ' not found!', mtWarning,
      [mbOK], 0);
  end
  else
  begin
...
begin
  ...
  begin
    Clear;
    for t := 0 to funcdesc.readinteger('Main', 'funccnt', 0) - 1 do
  ...
  begin
    ...
    begin
      ...
      WNDOutput(func_col1 + dospace(length(func_col1), 28) + func_col2 +
        dospace(length(func_col2), 28) + func_col3 +
        dospace(length(func_col3), 20));
      Inc(t, 3);
    end;
...
begin
  if outputwnd <> nil then
  begin
    WNDOutput(info.vars['text'].value);
  end
  else
  begin
    debugmsg('Print: ' + info.vars['text'].value);
  end;
end;
...
begin
  if outputwnd <> nil then
  begin
    if funcdesc.readstring('Functions', info.vars['cmd'].value, '') <> '' then
    begin
      WNDOutput('Syntax: ' + funcdesc.readstring('Functions',
        info.vars['cmd'].value + '_decl', ''));
      WNDOutput('');
      WNDOutput(funcdesc.readstring('Functions', info.vars['cmd'].value, ''));
    end
    else
      WNDOutput('Unknown command ' + info.vars['cmd'].value);
  end;
end;
...
procedure tform1.func_unitfunctionsshownodeeval(info: tprograminfo);
var
  s, ind: integer;
var
  node: dlgnode;
begin
  ind := getnodeindex(info.vars['name'].value);
  if ind = -1 then
    WNDOutput('Node "' + info.vars['name'].value + '" not found!')
  else
  begin
...
begin
  executeplugin(info.vars['name'].value);
end;
...
begin
  ...
  begin
    WNDOutput(IntToStr(t + 1) + ': ' + dllplugins[t].filename + ' (' +
      dllplugins[t].displayname + ')\nInterface Version: ' + dllplugins[t]
      .interfaceversion + '\nBuild date:    ' +
...
begin
  version_str := format('%d.%d.%d-%s', [version_major, version_minor,
    version_release, version_status]);
  WNDOutput('Fallout Dialogue Creator ' + version_str + ', Compiled on [' +
    builddate + ']');
end;
...
var
  Data: string;
begin
  field := info.vars['field'].value;
  Data := info.vars['data'].value;
  case field of
    field_npc_name:
      begin
    ...
    field_npc_detailed_look:
      begin
        WNDOutput(
          'SetDialogGlobals: Changed npc description_p_proc message to ' +
          Data);
...
begin
  savedata(info.vars['filename'].value);
  WNDOutput('Dialogue saved to ' + info.vars['filename'].value);
...
var
  thefile: string;
begin
  thefile := info.vars['filename'].value;
  zerrorstr := '';
  currentdlg.npcname := '';
  currentdlg.description := '';
  currentdlg.location := '';
  ...
  begin
    ...
    form1.statusbar1.simpletext := 'Dialogue ' + thefile + ', ' +
      IntToStr(currentdlg.nodecount) + ' nodes';
    currentfile := thefile;
  end;
end;
...
var
  t: integer;
begin
  t := getnodeindex(info.vars['node'].value);
  if t <> -1 then
  begin
...
begin
  ...
  begin
    ...
    for s := 0 to nodehistorysize - 1 do
    begin
      if nodehistoryoption[s] <> -1 then
        WNDOutput(format('Entry %0.3d %s\nNPC: %s\nPlayer: %s -> %s\n\n',
          ...
          currentdlg.nodes[nodehistorypreviousnode[s]].options[nodehistoryoption
          [s]].optiontext, currentdlg.nodes[nodehistorypreviousnode[s]].options
          [nodehistoryoption[s]].nodelink]))
      else
        WNDOutput(format('Entry %0.3d %s\nNPC: %s\n',
          [s + 1, currentdlg.nodes[nodehistory[s]].nodename,
          currentdlg.nodes[nodehistory[s]].npctext]));
...
begin
  info.vars['result'].value := dlgrunning;
  if dlgrunning = True then
...
begin
  ...
  begin
    WNDOutput(IntToStr(dlgtranscriptcnt) +
      ' dialogue transcript(s) available:\n');
    WNDOutput(
      'Use the number denoted by # to access the transcript with the ShowTranscript command.\n');
...
var
  z: integer;
begin
  ind := info.vars['num'].value;
  if ind > dlgtranscriptcnt then
  begin;
    WNDOutput('Transcript out of range. (max = ' + IntToStr(dlgtranscriptcnt -
      1) + ')');
  end
  else
  begin
...
procedure tform1.func_unitfunctionsflushdialogueeval(info: tprograminfo);
begin
...
procedure tform1.func_unitFunctionsFormattedIntEval(info: tprograminfo);
var
  formatstring: string;
var
  formattedindex: integer;
begin
  formattedindex := info.vars['formattedindex'].value;
  formatstring := info.vars['FormatString'].value;

  info.vars['Result'].value := format(formatstring, [formattedindex]);
end;

procedure tform1.func_unitfunctionscdeval(info: tprograminfo);
begin
  if directoryexists(info.vars['path'].value) then
  begin
    chdir(info.vars['path'].value);
    WNDOutput('Path is now: ' + getcurrentdir);
  end
  else
    WNDOutput(info.vars['path'].value + ': no such directory.');
end;
...
begin
  if dlgtranscriptcnt > 0 then
  begin
    if directoryexists(extractfiledir(ParamStr(0)) + '\dialogue\history') = False
    then
    begin
    ...
    begin
      ...
      try
        ...
        rewrite(s);
        for z := 0 to dlgtranscripts[t].linecnt - 1 do
        begin
          dlgtranscripts[t].Lines[z] :=
            stringreplace(dlgtranscripts[t].Lines[z], 'Player', player.Name,
            [rfreplaceall]);
          dlgtranscripts[t].Lines[z] :=
            stringreplace(dlgtranscripts[t].Lines[z], 'NPC', currentdlg.npcname,
            [rfreplaceall]);
        ...
        on e: exception do
        begin
          messagedlg(format('Error while saving dialog transcript #%d:' + #13 +
            #10 + '' + #13 + #10 + '%s' + #13 + #10 + '' + #13 + #10 +
            'Filename: %s', [z + 1, e.message, dlgtranscripts[t].histname]),
            mtWarning, [mbOK], 0);
        end;
      end;
    end;
...
procedure tform1.func_unitfunctionssavedialogtranscriptseval
  (info: tprograminfo);
begin
  ...
  begin
    ...
    begin
      ...
      tsyncompletionproposal(Sender)
        .itemlist.add(getitems(lookuplist.IndexOf(lookup)));
    end;
  end
  else
    tsyncompletionproposal(Sender).itemlist.Clear;
...
begin
  ...
  begin
    commands := commands + '<td><a href="ScriptFunctions.html#' + datanames[t] +
      '">' + datanames[t] + '</a></td>\n';
    Inc(t);
  end;
  commands := commands + '<tr>\n';
  ...
  begin
    ...
    func_col2 := stringreplace(func_col2, 'NOTE:',
      '<span class="note">NOTE:</span>', [rfreplaceall]);
    func_col2 := stringreplace(func_col2, ' : ', ': ', [rfreplaceall]);
    func_col2 := stringreplace(func_col2, ' String;', '<b> string</b>;',
      [rfreplaceall]);
    func_col2 := stringreplace(func_col2, ' String)', '<b> string</b>)',
      [rfreplaceall]);
  ...
  Data.Text := stringreplace(Data.Text, '%%InsertIndexHere%%', commands,
    [rfreplaceall]);
  Data.Text := stringreplace(Data.Text, '%%InsertFunctionsHere%%', Lines,
    [rfreplaceall]);
...
begin
  if fileexists(info.vars['Filename'].value) = False then
  begin
    WNDOutput(format('Warning: Supplied helper script "%s" does not exist.',
      [info.vars['filename'].value]));
  ...
  Inc(tempscnt);
  if tempscnt > 0 then
    form1.helperscripts1.Visible := True;
end;
...
begin
  thenodeindex := info.vars['Node'].value;
  if thenodeindex <> -1 then
  begin
    datafield := info.vars['Field'].value;
    case datafield of
      field_node_npctext:
        currentdlg.nodes[thenodeindex].npctext := info.vars['Newdata'].value;
      field_node_npcfemaletext:
        currentdlg.nodes[thenodeindex].npctext_female :=
          info.vars['Newdata'].value;
      field_node_notes:
        currentdlg.nodes[thenodeindex].notes := info.vars['Newdata'].value;
      field_node_name:
        currentdlg.nodes[thenodeindex].nodename := info.vars['Newdata'].value;
    end;
    refreshnodes;
    requestpluginrefresh;
  end;
...
begin
  ...
  begin
    ...
    case datafield of
      field_node_npctext:
        info.vars['result'].value := currentdlg.nodes[thenodeindex].npctext;
      field_node_npcfemaletext:
        info.vars['result'].value := currentdlg.nodes[thenodeindex]
          .npctext_female;
      field_node_notes:
        info.vars['result'].value := currentdlg.nodes[thenodeindex].notes;
      field_node_optioncount:
        info.vars['result'].value :=
          IntToStr(currentdlg.nodes[thenodeindex].optioncnt);
      field_node_name:
        info.vars['result'].value := currentdlg.nodes[thenodeindex].nodename;
    end;
  end;
end;
...
var
  x: integer;
begin
  if (info.vars['nodeIndex'].value >= 0) and
    (info.vars['nodeIndex'].value < currentdlg.nodecount - 1) then
  begin
    x := info.vars['nodeIndex'].value;
    info.vars['result'].value := currentdlg.nodes[x].nodename;
  end;
end;

procedure tform1.func_unitfunctionsgetplayeroptionstringdataeval
  (info: tprograminfo);
var
  node: string;
  thenode: integer;
  ...
  field: integer;
begin
  node := info.vars['Node'].value;
  option := info.vars['Option'].value;
  field := info.vars['field'].value;
  ...
  begin
    case field of
      field_playeroption_optiontext:
        begin;
          info.vars['result'].value := currentdlg.nodes[thenode].options[option]
            .optiontext;
        end;
      field_playeroption_nodelink:
        begin
          info.vars['result'].value := currentdlg.nodes[thenode].options
            [option].nodelink;
        end;
...
procedure tform1.func_unitfunctionsgetplayeroptionintegerdataeval
  (info: tprograminfo);
var
  node: string;
  thenode: integer;
...
procedure tform1.func_unitfunctionssetplayeroptionstringdataeval
  (info: tprograminfo);
var
  node: string;
  thenode: integer;
...
begin
  ...
  option := info.vars['Option'].value;
  field := info.vars['field'].value;
  the_value := info.vars['newdata'].value;
  thenode := getnodeindex(node);
  if thenode <> -1 then
  begin;
    case field of
      field_playeroption_optiontext:
        begin
          currentdlg.nodes[thenode].options[option].optiontext := the_value;
        end;
      field_playeroption_nodelink:
        begin
          currentdlg.nodes[thenode].options[option].nodelink := the_value;
        end;
      field_playeroption_notes:
        begin;
...
procedure tform1.func_unitfunctionssetplayeroptionintegerdataeval
  (info: tprograminfo);
var
  node: string;
  thenode: integer;
...
begin
  editoption(info.vars['Node'].value, info.vars['OptionNum'].value);
end;

procedure tform1.func_unitfunctionsgetnodecounteval(info: tprograminfo);
begin
  info.vars['result'].value := currentdlg.nodecount;
end;
...
var
  nodeind: integer;
begin
  nodeind := getnodeindex(info.vars['Name'].value);
  if nodeind <> -1 then
  begin
    deletenode(nodeind);
    requestpluginrefresh;
...
begin
  nodeind := getnodeindex(info.vars['Node'].value);
  if nodeind <> -1 then
  begin
    optionind := info.vars['OptionIndex'].value;
...
begin
  ...
  form15.jvspinedit7.value := player.luck;
...
begin
  if messagedlg(info.vars['Message'].value, mtConfirmation, [mbYes, mbNo], 0)
    in [mrYes] then
    info.vars['result'].value := True
  else
    info.vars['result'].value := False;
...
begin
  input := info.vars['srcString'].value;
  replaceall := info.vars['ReplaceAll'].value;
  if replaceall = True then
    replaceflags := [rfreplaceall]
  else
    replaceflags := [];
  input := stringreplace(input, info.vars['oldString'].value,
    info.vars['newString'].value, replaceflags);
  info.vars['result'].value := input;
end;
...
begin
  ...
  timeeventcommands[te_cmdcnt].scriptname := info.vars['script'].value;
  timeeventcommands[te_cmdcnt].menuitem := info.vars['Name'].value;
  timeeventcommands[te_cmdcnt].shortcut := info.vars['Shortcut'].value;
...
begin
  ...
  begin
    ...
    begin
      info.vars['result'].value := form20.listbox1.items
        [form20.listbox1.ItemIndex];
    end;
  end;
  form20.Caption := '';
  ...
  begin
    ...
    begin
      ...
      if currentdlg.timedevents[te_ind].israndom = True then
        info.vars['result'].value :=
          format('flush_add_timer_event_sec(self_obj, random(%d, %d),%s);',
        ...
        info.vars['result'].value :=
          format('flush_add_timer_event_sec(self_obj, %d,%s);',
...
var
  ...
  npctextfemale: string;
begin
  nodename := info.vars['nodename'].value;
  npctext := info.vars['npctext'].value;
  npctextfemale := info.vars['npctext_female'].value;
  notes := info.vars['notes'].value;
  if getnodeindex(nodename) <> -1 then
  begin;
    WNDOutput('Error! Node ' + nodename + ' already exists!');
    exit;
...
begin
  ...
  currentdlg.timedevents[currentdlg.timedeventcnt].mininterval :=
    info.vars['interval1'].value;
  currentdlg.timedevents[currentdlg.timedeventcnt].maxinterval :=
    info.vars['interval2'].value;
  currentdlg.timedevents[currentdlg.timedeventcnt].actioncnt := 0;
  WNDOutput('Added Timed Event: ' + currentdlg.timedevents
    [currentdlg.timedeventcnt].fixedparamname + ' ( index=' +
    IntToStr(currentdlg.timedevents[currentdlg.timedeventcnt]
    .fixedparamenumindex) + ' )');
...
var
  te_ind: integer;
begin
  te_ind := gettimedeventindex(info.vars['eventname'].value);
  if te_ind <> -1 then
  begin
    new(currentdlg.timedevents[te_ind].actionlines
      [currentdlg.timedevents[te_ind].actioncnt]);
    currentdlg.timedevents[te_ind].actionlines
      [currentdlg.timedevents[te_ind].actioncnt].linedata :=
      info.vars['codeline'].value;
    Inc(currentdlg.timedevents[te_ind].actioncnt);
    WNDOutput(
      'Added code line (currentdlg.timedevents[te_ind].actioncnt) to timed event "'
...
begin
  ...
  fnind := getfloatnodeindex(info.vars['nodename'].value);
  if fnind <> -1 then
  begin;
    WNDOutput('The specified float node already exists at index ' +
      IntToStr(fnind));
    exit;
  end;
  currentdlg.floatnodes[currentdlg.floatnodecount].nodename :=
    info.vars['nodename'].value;
  currentdlg.floatnodes[currentdlg.floatnodecount].notes :=
    info.vars['desc'].value;
  currentdlg.floatnodes[currentdlg.floatnodecount].messagecnt := 0;
  WNDOutput('Added floatnode "' + currentdlg.floatnodes
    [currentdlg.floatnodecount].nodename + '" (' + currentdlg.floatnodes
    [currentdlg.floatnodecount].notes + ')');
...
var
  ind: integer;
begin
  ind := getfloatnodeindex(info.vars['node'].value);
  if ind <> -1 then
  begin
...
begin
  fn_ind := getfloatnodeindex(info.vars['node'].value);
  if fn_ind <> -1 then
  begin
    nodeind := fn_ind;
    refreshfloatmsgs;
...
var
  te_ind: integer;
begin
  te_ind := gettimedeventindex(info.vars['event'].value);
  if currentdlg.timedevents[te_ind].israndom = True then
    info.vars['result'].value :=
      format('flush_add_timer_event_sec(self_obj, random(%d, %d),%s);',
      [currentdlg.timedevents[te_ind].mininterval,
...
var
  displaycmd: integer;
begin
  app := info.vars['app'].value;
  params := info.vars['params'].value;
  defaultdir := info.vars['defaultdir'].value;
  displaycmd := info.vars['displaycmd'].value;
...
var
  u: integer;
begin
  debugmsg('Loading definition ' + info.vars['fname'].value);
  incparser.sourcefilename := info.vars['fname'].value;
  incparser.Execute;
  // debugmsg('includes: '+incs[inc_Cnt].
  if incparser.successful = True then
  begin
  end
  else
  begin
    debugmsg('Parse failed!');
    incparser.ListStream.savetofile('debug_def.lst');
  end;
  // debugmsg('Inc count now: '+IntToStr(inc_cnt));
end;

procedure HelpSystem(parentform: TForm; filename: PChar;
  Caption: PChar); stdcall;
begin
...
var
  the_node: integer;
begin
  the_node := getnodeindex(info.vars['node'].value);
  new(currentdlg.nodes[the_node].skillchecks[currentdlg.nodes[the_node]
    .skillcheckcnt]);
  currentdlg.nodes[the_node].skillchecks
    [currentdlg.nodes[the_node].skillcheckcnt].check_proc_name :=
    info.vars['Name'].value;
  currentdlg.nodes[the_node].skillchecks
    [currentdlg.nodes[the_node].skillcheckcnt].check_what :=
    info.vars['skill'].value;
  currentdlg.nodes[the_node].skillchecks
    [currentdlg.nodes[the_node].skillcheckcnt].modifier :=
    info.vars['modifier'].value;
  currentdlg.nodes[the_node].skillchecks
    [currentdlg.nodes[the_node].skillcheckcnt].successnode :=
    info.vars['onsuccess'].value;
  currentdlg.nodes[the_node].skillchecks
    [currentdlg.nodes[the_node].skillcheckcnt].failurenode :=
    info.vars['onfailure'].value;
  Inc(currentdlg.nodes[the_node].skillcheckcnt);
end;

procedure tform1.func_unitfunctionssetoptiontoskillcheckeval
  (info: tprograminfo);
var
  nodeind, option: integer;
begin
  nodeind := getnodeindex(info.vars['node'].value);
  option := info.vars['opt_ind'].value;
  currentdlg.nodes[nodeind].options[option].link_to_proc := False;
  currentdlg.nodes[nodeind].options[option].link_to_skillcheck := True;
  currentdlg.nodes[nodeind].options[option].nodelink :=
    info.vars['skillcheck'].value;
end;
...
var
  x: integer;
begin
  x := getnodeindex(info.vars['Node'].value);
  if x <> -1 then
    currentdlg.nodes[x].customcode := info.vars['code'].value;
end;
...
exports HelpSystem, HelpSystemDeInit, getmainform, GetPlayer, GetProgramVer,
  SetOutputControl, GetCurrentNode, setcurrentnode, DoExternalRegister,
  UnsetOutputControl, SendDebugMSG, GetStyle, GetSSLPath, GetMSGPath,
  GetCurrentDLGName, GetDLG, GetDLGNew, GetDLGNew_Alt, ExecuteScriptLines;
...
begin
  ...
  filenamez := changefileext(currentdlg.filename, '') + '_backup_' +
    datestr + '.fmf';
  savedata(filenamez);
...
begin
  ...
  begin
    ...
    if pos(form1.replacedialog1.findtext, currentdlg.nodes[t].npctext_female) <> 0
    then
    begin
      Inc(occ);
      Inc(node);
    end;
    currentdlg.nodes[t].npctext := stringreplace(currentdlg.nodes[t].npctext,
      form1.replacedialog1.findtext, form1.replacedialog1.replacetext,
      [rfreplaceall]);
    ...
    for u := 0 to currentdlg.nodes[t].optioncnt - 1 do
    begin
      if pos(form1.replacedialog1.findtext, currentdlg.nodes[t].options[u]
        .optiontext) <> 0 then
      begin
        Inc(occ);
        Inc(playeropt);
  ...
  messagedlg(format('Search finished.' + #13 + #10 + '' + #13 + #10 +
    '%d total occurences:' + #13 + #10 + '' + #13 + #10 + '%d in npc text' + #13
    + #10 + '%d in player options', [occ, node, playeropt]), mtWarning,
    [mbOK], 0);
end;
...
begin
  progressform.progressbar1.max := info.vars['count'].value;
end;

procedure tform1.func_unitfunctionsupdateprogresseval(info: tprograminfo);
begin
  progressform.progressbar1.position := info.vars['pos'].value;
  application.ProcessMessages;
end;

procedure tform1.func_unitVariablesbasepathReadVar(info: tprograminfo;
  var value: Variant);
begin
  value := basepath;
end;
...
procedure tform1.func_unitfunctionssetprogressbarcaptioneval
  (info: tprograminfo);
begin
  progressform.label1.Caption := info.vars['text'].value;
  application.ProcessMessages;
end;
...
begin
  ...
  begin
    ...
    else if item.subitems[2] = 'Good' then
    begin
      listbox1.canvas.font.color := good_col;
      listbox1.canvas.font.Style := [fsbold];
    end
    else if item.subitems[2] = 'Bad' then
    begin
...
var
  ind: integer;
begin
  ind := info.vars['index'].value;
  info.vars['result'].value := (currentdlg.nodes[ind].hidden = True);
end;
...
begin
  ...
  for i := 0 to currentdlg.nodes[src_node].options[src_opt].conditioncnt - 1 do
  begin
...
var
  ...
  i: integer;
begin
  src_node := info.vars['SourceNodeIndex'].value;
  dest_node := info.vars['DestinationNodeIndex'].value;
  src_opt := info.vars['SourceConditionPlayerOption'].value;
  dest_opt := info.vars['TargetConditionPlayerOption'].value;
