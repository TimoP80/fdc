...
type
  TForm16 = class(TForm)
    ...
    procedure condlistMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
...
begin
  ...
  sslenum := condcfg.ReadString(checktype, 'Item' + IntToStr(fieldlist.ItemIndex + 1) + 'SSLEnum', '');
...
begin
  ...
  begin
    ...
    for h := 1 to itemcount do
      fieldlist.Items.Add(condcfg.ReadString(checktype, 'Item' + IntToStr(h), ''));
...
begin
  ...
  begin
    ;
    form16.condlist.items.add(
      currentdlg.nodes[nodeind].options[lineind].conditions[t].resolved_code);
  end;
...
begin
  ...
  new(currentdlg.nodes[nodeind].options[lineind].conditions[currentdlg.nodes[nodeind].options[lineind].conditioncnt]);
  ...
  if (Source = 'local_vars') or (Source = 'script_vars') or (Source = 'global_vars') then
  begin
    currentdlg.nodes[nodeind].options[lineind].conditions[
      currentdlg.nodes[nodeind].options[lineind].conditioncnt].check_field :=
      fieldlist.ItemIndex;
    ...
    currentdlg.nodes[nodeind].options[lineind].conditions[
      currentdlg.nodes[nodeind].options[lineind].conditioncnt].check_field :=
      condcfg.ReadInteger(checktypelist.Items[checktypelist.ItemIndex], 'Item' + IntToStr(fieldlist.ItemIndex + 1) + 'Enum', 0);
  currentdlg.nodes[nodeind].options[lineind].conditions[
  ...
  currentdlg.nodes[nodeind].options[lineind].conditions[
    ...
    sslcode.Caption;
  currentdlg.nodes[nodeind].options[lineind].conditions[
    currentdlg.nodes[nodeind].options[lineind].conditioncnt].link := linkage.ItemIndex;
...
begin
  ...
  if condlist.ItemIndex <> -1 then
  begin
    curcond := currentdlg.nodes[nodeind].options[lineind].conditions[condlist.ItemIndex]^;
    checktypelist.ItemIndex := curcond.check_type - 1;
    checktypelistClick(nil);
    if (Source = 'defines') then
    begin
      debugmsg('DEFINES! VARPTR=' + curcond.var_ptr);
      fieldlist.ItemIndex := fieldlist.items.indexof(GetDefineValueByEnum(curcond.var_ptr));
    ...
    begin
      ...
      begin
        fieldlist.ItemIndex := 0;
        MessageDlg(format('Warning: The variable "%s" contained in the condition does not resolve' + #13 +
          #10 + 'to a valid script variable.' + #13 + #10 + '' + #13 + #10 +
          'It has been reset to the first variable in the list to prevent crashing.', [curcond.var_ptr]),
          mtWarning, [mbOK], 0);
      end;
...
begin
  try
    ...
    begin
      currentdlg.nodes[nodeind].options[lineind].conditions[
        condlist.ItemIndex].check_type :=
        checktypelist.ItemIndex + 1;
      Data := condcfg.ReadInteger(checktypelist.Items[checktypelist.ItemIndex], 'Item' +
        IntToStr(fieldlist.ItemIndex + 1) + 'Enum', 0);
      currentdlg.nodes[nodeind].options[lineind].conditions[
        condlist.ItemIndex].check_field := Data;
      currentdlg.nodes[nodeind].options[lineind].conditions[
        condlist.ItemIndex].check_eval := evaltype.ItemIndex;
      currentdlg.nodes[nodeind].options[lineind].conditions[
        condlist.ItemIndex].check_value := valuetocheck.Text;
      ...
      debugmsg('VarPTR := ' + currentdlg.nodes[nodeind].options[lineind].conditions[condlist.ItemIndex].var_ptr);
      currentdlg.nodes[nodeind].options[lineind].conditions[
        condlist.ItemIndex].resolved_code := sslcode.Caption;
      currentdlg.nodes[nodeind].options[lineind].conditions[condlist.ItemIndex].link :=
        linkage.ItemIndex;
    ...
    on e: Exception do
    begin
      MessageDlg('SHIT GOD DAMN CRAP FUCK IT!' + #13 + #10 + '' + #13 + #10 + 'Okay.. Some shit went wrong.', mtWarning, [mbOK], 0);
    end;
  end;
...
procedure TForm16.condlistMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
