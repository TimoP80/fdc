...
{$APPTYPE CONSOLE}
uses
  Classes, arcmsg, parseholodisk,CocoBase, HSParser, SysUtils;
const
...
var
  ...
  begin;
    ...
    begin;
      ...
      begin;
        ...
        if extractfileext(newfile)='' then newfile := newfile + '.hs';
  ...
  procedure ShowHelp;
  begin
    Writeln('Usage: HoloCMP [filename]');
    Writeln('Example: HoloCMP test.hs');
  end;
...
current_title_id := 400;
     ...
     parse_mes('pipboy_empty.msg',pipboydata);
      ...
      begin;
      for x:=0 to SourceList.Count-1 do
      begin;
      writeln('parsing ',sourcelist[x]);
      HSParser1.SourceFileName := sourcelist[x];
      HSParser1.Execute;
...
try
   ...
   if hsparser1.successful then
   begin;
   SaveHolos('holodisk.txt');
   save_mesfile('pipboy.msg',pipboydata);
   end;
